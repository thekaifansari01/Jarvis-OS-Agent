...
from tools.Vision.screen_capture import capture_screen
...
def execute_actions(result: Dict[str, any], executor: ThreadPoolExecutor) -> str:
    ...
    try:
        ...
        quick_search_query = result.get('quick_web_search')
        if quick_search_query:
            def run_quick_search(query):
                log_action(f"🔍 Fast Brain Searching: {query}")
                try:
                    from tools.SearchTools.WebSearch import search_web
                    search_result = search_web(query, max_results=1) 
                    
                    if "AI QUICK SUMMARY" in search_result:
                        summary = search_result.split("💡 AI QUICK SUMMARY (Use this for direct facts):\n")[1].split("=")[0].strip()
                        log_action(f"🤖 JARVIS: {summary}")
                        executor.submit(speak, summary)
                    else:
                        executor.submit(speak, "Sir, mujhe search results se seedha answer nahi mila. Aap deep research ke liye bol sakte hain.")
                except Exception as e:
                    logger.error(f"❌ Quick search failed: {e}")
                    executor.submit(speak, "Sorry sir, search karne mein network error aa gaya.")
            
            executor.submit(run_quick_search, quick_search_query)
        ...
        if result.get('system_action'):
            def sys_act():
                ...
                if action == 'lock':
                    ...
                    msg = SystemController.capture_screenshot()
                    log_action(f"📸 {msg}")
...
def execute_single_tool_sync(action_dict: Dict[str, any]) -> str:
    ...
    vision_cmd = action_dict.get('vision_action')
    if vision_cmd and isinstance(vision_cmd, dict) and vision_cmd.get('action') == 'capture_screen':
        try:
            logger.info("🤖 Agent executing Vision: Capturing screen for analysis")
            img_path = capture_screen()
            if img_path: 
                logger.info(f"✅ Screenshot captured: {img_path}")
                return f"Observation: Screenshot captured successfully at {img_path}. Please analyze this image in the next step to answer the user's query."
            else: 
                logger.error("❌ Failed to capture screenshot.")
                return "Observation: Failed to capture screenshot. Tell the user you cannot see the screen right now."
        except Exception as e:
            logger.error(f"❌ Vision Action crashed: {e}\n{traceback.format_exc()}")
            return f"Observation: Vision tool error -> {e}"
