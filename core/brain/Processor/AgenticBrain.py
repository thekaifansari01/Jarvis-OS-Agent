            ...
            if ai_response:
                for key, value in ai_response.items():
                    if key not in ignore_keys:
                        ...
                        elif key == "vision_action" and isinstance(value, dict):
                    ...
                    if observation:
                        ...
                        if action_key == "workspace_action" and ai_response.get("workspace_action", {}).get("action") == "read":
                            ...
                            if target_file.lower().endswith(('.png', '.jpg', '.jpeg', '.webp')):
                                match = re.search(r'at (.*?)\. Please', str(observation))
                                if match and os.path.exists(match.group(1).strip()):
                                    pending_image_to_read = match.group(1).strip()
                                    logger.info(f"📸 Smart Trigger: Agent requested workspace image. Scheduled: {pending_image_to_read}")

                        elif action_key == "vision_action" and ai_response.get("vision_action", {}).get("action") == "capture_screen":
                            match = re.search(r'at (.*?)\. Please', str(observation))
                            if match and os.path.exists(match.group(1).strip()):
                                pending_image_to_read = match.group(1).strip()
                                logger.info(f"📸 Smart Trigger: Agent requested screen capture. Scheduled: {pending_image_to_read}")
