...
class ContextMemory:
    ...
    def _extract_insights_ai(self, message):
        ...
        try:
            ...
            2. The Assistant Rule: Only save things that an AI Assistant needs to know.
            3. No Duplicates / Update Rule: Review the [EXISTING KNOWLEDGE] block. DO NOT extract facts we already know.
            4. Mood Tracker: ALWAYS detect the current emotional 'mood'.

            [EXISTING KNOWLEDGE]
            Known Bio Facts: {[f.get("text", "") for f in self.user_bio.get("facts", [])]}
            ...
            CATEGORIES:
            1. 'bio': Hard, unchanging facts.
            2. 'prefs': Actionable preferences.
            ...
            if insights.get("bio") and len(insights["bio"]) > 0:
                for fact in insights["bio"]:
                    if not any(f.get("text", "").lower() == fact.lower() for f in self.user_bio.get("facts", [])):
                        self.user_bio.setdefault("facts", []).append({"text": fact, "date": datetime.now().isoformat()})
                        updated = True
            ...
            if insights.get("prefs") and len(insights["prefs"]) > 0:
                for pref in insights["prefs"]:
                    if not any(p.lower() == pref.lower() for p in self.preferences.get("likes", [])):
                        self.preferences.setdefault("likes", []).append(pref)
                        updated = True
                if len(self.preferences.get("likes", [])) > 20: 
                    self.preferences["likes"] = self.preferences["likes"][-20:]
            ...
            if current_mood and current_mood.lower() != "neutral":
                ...
                self.user_mood.setdefault("mood_history", []).append(mood_entry)
