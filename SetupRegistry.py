...
"""
    try:
        ...
        logger.info("SUCCESS: Automatically generated 'setup.py'.")
        return True
    except Exception as e:
        logger.error(f"ERROR: Failed to create 'setup.py': {e}")
        return False
...
def register_custom_protocol(current_dir):
    ...
    if not os.path.exists(handler_script):
        logger.warning(f"'JarvisProtocol.py' not found at {handler_script}.")
    ...
    try:
        ...
        logger.info("SUCCESS: URL Protocol 'jarvis://' registered.")
        return True
    except PermissionError:
        logger.error("Permission denied while writing to Windows Registry. Run as Administrator.")
        return False
    except Exception as e:
...
def install_cli_package(current_dir):
    ...
    if not os.path.exists(setup_file):
        logger.error("'setup.py' missing. Cannot build CLI command.")
        return False

    try:
        logger.info("Building 'jarvis' CLI command inside virtual environment...")
        python_exe = sys.executable
        ...
        if result.returncode == 0:
            logger.info("SUCCESS: 'jarvis' CLI command installed.")
            return True
        else:
            logger.error(f"pip install failed:\n{result.stderr.strip()}")
            return False
    except FileNotFoundError:
        logger.error("Python executable or pip module not found.")
        return False
    except Exception as e:
        logger.error(f"Unexpected error during CLI installation: {e}")
        return False
...
def add_venv_to_system_path(current_dir):
    ...
    if not os.path.exists(venv_scripts_path):
        logger.warning(f"Virtual environment folder not found at: {venv_scripts_path}")
        return False
    ...
    try:
        ...
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0, winreg.KEY_READ | winreg.KEY_WRITE) as key:
            ...
            logger.info(f"SUCCESS: Added to Windows PATH -> {venv_scripts_path}")
            return True

    except PermissionError:
        logger.error("Permission denied while updating environment variables.")
        return False
    except Exception as e:
        logger.error(f"Failed to update Windows PATH: {e}")
        return False

def main():
    logger.info("Initializing Smart Jarvis Setup System...")
    logger.info("=" * 50)
    ...
    try:
        ...
        if not in_venv:
            logger.warning("Virtual Environment (.venv) is NOT activated.")
            if reg_setup:
                logger.info("URL Protocol ('jarvis://') is ALREADY configured in Windows Registry.")
                logger.info("ACTION REQUIRED: To configure the CMD 'jarvis' command, activate '.venv' and run this script again.")
            else:
                logger.info("Setting up URL Protocol ('jarvis://') only...")
        ...
        if reg_setup and cli_installed and path_added:
            logger.info("URL Protocol ('jarvis://') is already configured.")
            logger.info("'jarvis' CLI command is already installed.")
            logger.info("'.venv\\Scripts' is already present in Windows PATH.")
            logger.info("=" * 50)
            logger.info("ALL COMPONENTS ARE ALREADY CONFIGURED! No changes needed.")
            return

        logger.info("Active Virtual Environment detected. Verifying components...")
        ...
        logger.info("=" * 50)
        logger.info("SETUP COMPLETED! Open a NEW Terminal window and type: jarvis")
