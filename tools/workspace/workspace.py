...
class WorkspaceManager:
    ...
    def sync_registry(self):
        ...
        if changed:
            ...
            logger.info("売 Workspace Two-Way Sync Complete.")
            
            try:
                import threading
                from core.brain.RagEngine import rag_engine
                threading.Thread(target=rag_engine._index_vault_files, daemon=True).start()
                logger.info("🔄 Shadow Sync: Triggered RAG Indexing for Workspace updates.")
            except Exception as e:
                logger.error(f"Failed to trigger RAG Shadow Sync: {e}")
