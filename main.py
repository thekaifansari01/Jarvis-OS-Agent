    ...
    try:
        ...
        if os.path.exists(script_path):
            if not os.path.exists(session_creds_path):
                logging.info("📱 WhatsApp Session nahi mila. QR Code scanning ke liye naya terminal khol raha hoon...")
            ...
            logging.info("✅ Node.js Baileys Server verification complete.")
        else:
            logging.warning(f"Baileys server script not found at {script_path}")
    except Exception as e:
        logging.warning(f"Could not start Baileys server: {e}")
...
def stop_baileys_server():
    ...
    if _baileys_process:
        try:
            ...
            pass
        _baileys_process = None
        logging.info("🛑 Baileys Server stopped")
